...
scd TestSCD {
    system Core {}
    external User {}
    flow Request(UndefinedType): User -> Core
}
"""
...
dfd TestDFD {
    ...
    process Handle {}
    process Other {}
    flow Context.Request: -> Handle
    flow Undefined(UndefinedType): Handle -> Other
}
"""
        doc = TextDocument(uri="file:///test.dit", source=source)
        diagnostics = _get_diagnostics(doc)
...
"""
        ...
        # Line 4: "    flow Request(UndefinedType): User -> Core"
        # Line 5: "}"
    ...
    external A {}
    external B {}
    flow FlowA(TypeA): A -> Core
    flow FlowB(TypeB): Core -> B
}
"""
...
"""
        # The DFD internal flow is on line 14 (1-based), so LSP line should be 13 (0-based)
        doc = TextDocument(uri="file:///test.dit", source=source)
        diagnostics = _get_diagnostics(doc)
        ...
        assert dfd_errors[0].range.start.line == 13, (
            f"Expected line 13 (0-based), got {dfd_errors[0].range.start.line}"
        )
    ...
    def test_valid_document_no_error_diagnostics(self) -> None:
        ...
        source = """
datadict {
    ValidType = { data: string }
}
scd TestSCD {
    system Core {}
    external User {}
    flow ValidFlow(ValidType): User -> Core
}
"""
        doc = TextDocument(uri="file:///test.dit", source=source)
        diagnostics = _get_diagnostics(doc)
...
"""
        ...
        flow_diagnostics = [d for d in diagnostics if "Request" in d.message]
